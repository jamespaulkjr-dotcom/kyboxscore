...
`/robots.txt` and `/sitemap.xml` are generated by `apps/web/app/robots.ts` and
`sitemap.ts`. Everything readable without a login is open; `/admin/`,
`/coach/`, `/score/`, `/account/`, `/login` and `/api/` are disallowed.
`/score/` matters most: those URLs are handed to a scorekeeper for one night
and a crawler has no business following one.

The sitemap lists every team page and every non-scrimmage game across open
seasons, with `lastModified` from the record or the game's `updated_at`, so a
crawler comes back for a score that changed. Deleted games are absent for free,
because `game` is a view over the ones that still exist.

## Works without JavaScript

Every page a reader looks at is server-rendered and complete without
JavaScript: scores, standings, RPI, team pages, game pages, search, about. That
is not a nicety, it is the performance budget — a parent on bad LTE in a gym
lobby gets HTML, not a loading spinner.

Four things need it, and all four degrade rather than break:

| Feature | Without JavaScript |
|---|---|
| Follow a team | The star renders signed-out; following is a browser-stored list |
| Live score polling | You get the score as it was when the page loaded |
| "Admin" in the header | Reads a cookie in the browser, so it says "Sign in" |
| Scoring console | Every form posts; the quarter pills and shared clock fall back to per-form controls in `<noscript>` |
